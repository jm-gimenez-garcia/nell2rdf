...
public class NellOntologyConverter {
	...
	public static final String	NDPROPERTIES		= "NDPROPERTIES";
	 ...
	 *
	 * @param csvFileName
	 *            comma-separated Nell ontology file as generated by CMU team.
	 * @param prefix
	 *            prefix to the ontology.
	 * @param rdfFileName
	 *            Absolute path to the turtle file where to serialize the Jena ontology as turtle on the filesystem.
	 */
	public NellOntologyConverter(final String format, final String metadata, final String csvFileName, final String rdfFileName) {
		super();
		this.format = format;
	...
	public Model convert() {
		...
		this.nellOntologyModel.createModel(this.ontologyFile);
		log.info("Done.");

		log.info("Mapping NELL ontology to a RDF model ...");
		final Model nellInRDF = this.nellOntologyModel.mapToRDFModel(this.prefix);
		log.info("Done.");
		return nellInRDF;
	}
	...
	private static String usage() {
		...
		sb.append("2- Desired model to represent meta-data: \"none\", \"reification\", \"n-ary\", \"quads\", \"singleton-property\", \"ndfluents\", \"ndproperties\"\n");
		sb.append("3- Absolute path to nell csv ontology file. E.g. \"./NELL.08m.1075.ontology.csv\"\n");
		sb.append("4- Absolute path where to serialize the RDF model. E.g. \"./NELL.08m.1075.ontology.ttl\"\n");
		sb.append("5- Absolute path to nell csv instance file. E.g. \"./NELL.08m.1070.cesv.csv\"\n");
		sb.append("6- Absolute path where to serialize the RDF instances. E.g. \"./NELL.08m.1070.cesv.ttl\"\n");
		sb.append("7- Where to delete original triple (optional, true by default, not applicable for quads)");
		return sb.toString();
	}
	 ...
	 *
	 * @param args
	 *            Array of program argument as strings.
	 * @return true if the number of arguments equals 5 and the Nell csv files (args[2] and args[4] values) exists on the filesystem.
	 */
	private static boolean areSuppliedArgumentsOK(final String[] args) {

		if (args.length != 6 && args.length != 7) { return false; }

		final File f = new File(args[2]);
		final File n = new File(args[4]);
	 ...
	 *            <li>args[2] : absolute path to nell ontology file on your FS.</li>
	 *            <li>args[3] : absolute path to the output turtle file to create on your FS.</li>
	 *            <li>args[4] : absolute path to the nell file on your FS.</li>
	 *            <li>args[5] : absolute path to the output turtle file for nell to create on your FS.</li>
	 *            </ul>
	 */
	public static void main(final String[] args) {
		...
		final String ontologyFile = args[2];
		final String outputFile = args[3];
		final String nellFile = args[4];
		final String outputNellToRDF = args[5];
		final String deleteOriginalTriples = args.length == 7 ? args[6] : "true";

		// create an in-memory model of the NELL csv file.
		log.info("Build an in-memory model of Nell ontology as a CSV file...");
		final NellOntologyConverter converter = new NellOntologyConverter(format, metadata, ontologyFile, outputFile);
		log.info("Done.");

		// convert this model to a Jena model.
		log.info("Converting the Nell in-memory model to a Jena model...");
		final Model nellRdfOntology = converter.convert();
		log.info("Done.");
		 ...
		 */
		NellMapper.displayStats(nellRdfOntology, converter.getNellOntologyModel());
		...
		converter.translateNellInstancesToRDF(nellFile, outputNellToRDF, Boolean.parseBoolean(deleteOriginalTriples));
	 ...
	 *
	 * @param outputFile
	 *            Absolute path of the file where to serialize the model.
	 */
	 ...
	 *
	 * @param nellInstanceFile
	 *            Absolute path to a Nell instance file
	 * @param rdfInstanceFile
	 *            Absolute path to where to dump instances on the FS
	 * @param prefix
	 *            Prefix to use when creating new IRIs.
	 * @param separator
	 */
	private void translateNellInstancesToRDF(final String nellInstanceFile, final String rdfInstanceFile, final boolean deleteOriginalTriples) {
		...
		if (!this.format.equals(XML)
				&& !this.format.equals(XML_ABBREV)
				&& !this.format.equals(NTRIPLES)
				&& !this.format.equals(TURTLE)
				&& !this.format.equals(TTL)
				&& !this.format.equals(N3)
				&& !this.format.equals(QUADS)) {
			this.format = NTRIPLES;
		}

		log.info("Exporting triples using " + this.format);

		// workaround to use less memory
		final ExtractNell extract = new ExtractNell(this.metadata, this.format, rdfInstanceFile, deleteOriginalTriples);
		final WriteNell write;
	...
	public NellModel getNellOntologyModel() {
		return this.nellOntologyModel;
	}

	public void setNellOntologyModel(final NellModel nellOntologyModel) {
